           		...
           		<div class="banner_inner d-flex align-items-center">
					<div class="banner_content">
						<div class="media">
							...
							<!--textbox-->
							<form action="q_save-pet.php" name="q_save-pet" method="post" enctype="multipart/form-data">
								<div class="col-12 col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
								<h2 align="center">เพิ่มข้อมูลสัตว์เลี้ยง</h3><hr><br>
								  <div class="container">
									<div class="row">
								<div class="col-12 col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
								<label>ชื่อสัตว์เลี้ยง</label>
								<input type="text" name="pet_name" class="form-control" placeholder="กรุณากรอกข้อมูล"/><br>
								<label>เพศ</label><br>
								...
								<input type="text" name="pet_date" class="form-control" placeholder="กรุณากรอกข้อมูล"/><br>
								<label>สายพันธ์สัตว์เลี้ยง</label>
								<input type="text" name="pet_species" class="form-control" placeholder="กรุณากรอกข้อมูล"/><br>
								<label>ชื่อเจ้าของ</label><br>
								   <select name="per_id" class="form-control dropdown_group" id="validationpet" required>
										<option value=""><-- กรุณาเลือกเจ้าของสุนัข --></option>
										<?php
										$sqlper_id = "SELECT * FROM person ORDER BY per_id ";
										$queryper_id = mysqli_query($conn,$sqlper_id);
										while($rs_per=mysqli_fetch_array($queryper_id,MYSQLI_ASSOC))
										{
										?>
										<option value="<?php echo $rs_per["per_id"];?>"><?php echo $rs_per["per_name"];?></option>
										<?php
										}
										?>
										</select><br><br>
								<label>รูปภาพสัคว์เลี้ยง</label>
								<input type="file" name="pet_img" class="form-control" placeholder="กรุณากรอกข้อมูล"/>
								</div>
								</div>
								</div>
								</div><br><br>
								...
								<input type="submit" class="from-control btn btn-success" name="a_save-pet" value="บันทึกข้อมูล" style="width:100%"/>
								</div>
								<div class="col-6 col-xs-6 col-sm-6 col-md-6 col-lg-6 col-xl-6">
								<input type="reset" class="from-control btn btn-danger" name="a_reset-pet" value="เคลียร์ข้อมูล" style="width:100%"/>
								</div>
								</div>
								</div>
								</div><br>
